...
ConstantValue* compileConstant2(void) {
  ...
  case TK_NUMBER:
    ...
    break;
  case TK_IDENT:
    constValue = lookupObject(lookAhead->string)->constAttrs->value;
    eat(TK_IDENT);
    break;
  default:
    constValue = duplicateConstantValue(NULL);
    error(ERR_INVALID_CONSTANT, lookAhead->lineNo, lookAhead->colNo);
    break;
  }
  return constValue;
...
Type* compileType(void) {
  ...
  switch (lookAhead->tokenType) {
    ...
    case KW_ARRAY:
      ...
      break;
    case TK_IDENT:
      type = lookupObject(lookAhead->string)->typeAttrs->actualType;
      // printf("%s", type);
      eat(TK_IDENT);
      break;
    default:
